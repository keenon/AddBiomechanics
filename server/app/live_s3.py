...
class LiveS3File:
  ...
  def __init__(self, root: LiveS3, name: str, path: str, parent: LiveS3File):
    ...
    self.path = self.path[1:] if len(self.path) > 0 and self.path[0] == '/' else self.path
    self.children = {}
  ...
  def uploadFile(self, filePath: str):
    ...
    self.root.s3.Object(BUCKET_NAME, self.path).put(Body=open(filePath, 'rb'))
    self.root.pubSub.sendMessage("/UPDATE/"+self.parentPath, {'path': self.path})
